---
title: Роутинг в Rails
description: Это руководство охватывает открытые для пользователя функции роутинга Rails.
---
...
## Ресурсный роутинг: по умолчанию в Rails
...
<Callout type="info">
Чтобы найти имена маршрутных хелперов для ваших маршрутов, смотрите [Список существующих маршрутов](#listing-existing-routes) ниже.
</Callout>
...
<Callout type="info">
Вызов `resolve` необходим для преобразования экземпляров `Geocoder` в маршруты через [идентификацию записи](/docs/form_helpers#relying-on-record-identification).
</Callout>
...
<Callout type="info">
`GET` в Rails не проверяет токен CSRF. Никогда не пишите в базу данных из `GET` запросов, подробнее о контрмерах CSRF смотрите в руководстве [Безопасность приложений на Rails](/docs/security#csrf-countermeasures).
</Callout>
...
Также можно ограничить маршрут, основываясь на любом методе в [объекте Request](/docs/action_controller_overview#the-request-and-response-objects), который возвращает `String`.
...
Вместо строки, подобной `'articles#index'`, соответствующей экшну `index` в `ArticlesController`, можно определить любое [приложение Rack](/docs/rails_on_rack) как конечную точку совпадения.
...
<Callout type="info">
Так как proc/lambda это объекты, отвечающие на `call`, можно реализовывать очень простые маршруты (например, для проверки здоровья) в одну строку:<br />`get '/health', to: ->(env) { [204, {}, ['']] }`
</Callout>
