...
<Callout type="info">
Вызов `resolve` необходим для преобразования экземпляров `Geocoder` в маршруты через [идентификацию записи](/docs/form-helpers#relying-on-record-identification).
</Callout>
...
Также можно ограничить маршрут, основываясь на любом методе в [объекте Request](/docs/action-controller-overview#the-request-and-response-objects), который возвращает `String`.
...
Вместо строки, подобной `'articles#index'`, соответствующей экшну `index` в `ArticlesController`, можно определить любое [приложение Rack](/docs/rails-on-rack) как конечную точку совпадения.
