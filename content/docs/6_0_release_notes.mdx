---
title: Заметки о релизе Ruby on Rails 6.0
description: "Ключевые новинки в Rails 6.0:"
---
...
[Action Mailbox](https://github.com/rails/rails/tree/6-0-stable/actionmailbox) позволяет направлять входящие письма в подобные контроллерам почтовые ящики. Подробнее об Action Mailbox можно прочитать в руководстве [Основы Action Mailbox](/docs/action-mailbox-basics).
...
Подробнее об Action Text можно прочитать в руководстве [Обзор Action Text](/docs/action-text-overview).

### Параллельное тестирование

[Pull Request](https://github.com/rails/rails/pull/31900)

[Параллельное тестирование](/docs/testing#parallel-testing) позволяет распараллелить тестовый набор. Хотя форк процессов является методом по умолчанию, треды также поддерживаются. Запуск тестов параллельно сокращает время, затрачиваемое на запуск всего тестового набора.

### Тестирование Action Cable

[Pull Request](https://github.com/rails/rails/pull/33659)

[Инструменты тестирования Action Cable](/docs/testing#testing-action-cable) позволяют тестировать функционал Action Cable на любом уровне: соединения, каналы, трансляции.
