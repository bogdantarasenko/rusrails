---
title: Основы Action Mailer
description: Это руководство представит вам все, что нужно для того, чтобы отправлять электронную почту в вашем приложении, и раскроет множество внутренних методов Action Mailer. Оно также раскроет, как тестироват
---
...
Подробнее читайте в руководстве [Кэширование с Rails: Обзор](/docs/caching_with_rails).
...
Тогда предпросмотр будет доступно по адресу [http://localhost:3000/rails/mailers/user_mailer/welcome_email](http://localhost:3000/rails/mailers/user_mailer/welcome_email).

Если вы поменяете что-то в `app/views/user_mailer/welcome_email.html.erb` или в самом рассыльщике, это автоматически перезагрузится и отрендерится, таким образом, вы можете увидеть новые стили мгновенно. Список для предпросмотра также доступен по адресу [http://localhost:3000/rails/mailers](http://localhost:3000/rails/mailers).
...
<Callout type="info">
не `GET` ссылки требуют [rails-ujs](https://github.com/rails/rails/blob/main/actionview/app/assets/javascripts) или [jQuery UJS](https://github.com/rails/jquery-ujs)
</Callout>
...
Подробное описание возможных конфигураций смотрите в [разделе про настройку Action Mailer](/docs/configuring#configuring-action-mailer) нашего руководства по конфигурированию приложений на Rails.

[`config.action_mailer.sendmail_settings`]: /docs/configuring#config-action-mailer-sendmail-settings
[`config.action_mailer.smtp_settings`]: /docs/configuring#config-action-mailer-smtp-settings
...
<Callout type="info">
Вышеприведенный пример использует пользовательское окружение по имени "staging" для сервера, похожего на production, но для целей тестирования. Подробнее о пользовательских окружениях в Rails можно прочитать в [Создание сред Rails](/docs/configuring#creating-rails-environments).
</Callout>
