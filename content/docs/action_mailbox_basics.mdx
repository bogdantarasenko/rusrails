...
Action Mailbox маршрутизирует входящие письма в подобные контроллеру ящики для обработки в приложении Rails. Action Mailbox предназначен для получения электронных писем, в то время как [Action Mailer](/docs/action_mailer_basics) используется для их *отправки*.

Входящие электронные письма асинхронно направляются с помощью [Active Job](/active_job_basics) в один или несколько специализированных почтовых ящиков. Затем эти письма преобразуются в записи [`InboundEmail`](https://api.rubyonrails.org/classes/ActionMailbox/InboundEmail.html) с помощью [Active Record](/docs/active_record_basics), которые могут взаимодействовать напрямую с остальными моделями вашей предметной области.
