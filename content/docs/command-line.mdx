...
<Callout type="info">
Этот самоучитель предполагает, что вы обладаете знаниями основ Rails, которые можно почерпнуть в руководстве [Rails для начинающих](/docs/getting-started).
</Callout>
...
<Callout type="info">
Гем rails можно установив, написав `gem install rails`, если вы еще этого не сделали.
</Callout>
...
<Callout type="info">
Также можно использовать псевдоним "g" для вызова команды `generate`: `bin/rails g`.
</Callout>
...
<Callout type="info">
Все консольные утилиты Rails имеют текст помощи. Как и с большинством утилит \*nix, можно попробовать `--help` или `-h` в конце, например `bin/rails server --help`.
</Callout>
...
<Callout type="info">
Список доступных типов полей для параметра `type` можно узнать в [документации API](https://api.rubyonrails.org/classes/ActiveRecord/ConnectionAdapters/SchemaStatements.html#method-i-add_column) для метода add_column модуля `SchemaStatements`. Параметр `index` генерирует соответствующий индекс для столбца.
</Callout>
...
Более подробно о миграциях написано в руководстве [Миграции Active Record](/docs/active-record-migrations).
...
<Callout type="info">
Хорошее описание юнит-тестирования в Rails дано в руководстве [Тестирование приложений на Rails](/docs/testing).
</Callout>
