---
title: Связи (ассоциации) Active Record
description: Это руководство раскрывает особенности связей Active Record.
---
...
<Callout type="info">
Опция `to_table`, передаваемая в `foreign_key`, подробнее объяснена в [`SchemaStatements#add_reference`][connection.add_reference].
</Callout>
...
[foreign_keys]: /docs/active_record_migrations#foreign-keys
...
* Проверяет [наличие](/docs/active_record_validations#presence) и [отсутствие](/docs/active_record_validations#absence) связей в большинстве случаев:
...
[`config.active_record.automatic_scope_inversing`]: /docs/configuring#config-active-record-automatic-scope-inversing
...
Внутри блока скоупа можно использовать любые стандартные [методы запросов](/docs/active_record_querying). Далее обсудим следующие из них:
...
<Callout type="warn">
Объекты будут _всегда_ удаляться из базы данных, игнорируя опцию `:dependent`.
</Callout>
...
Подробнее о колбэках связи читайте в [Руководстве о колбэках Active Record](/docs/active_record_callbacks.html#association-callbacks)
