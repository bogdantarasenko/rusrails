---
title: Апгрейд Ruby on Rails
description: В этом руководстве приведены шаги, которые необходимо выполнить, чтобы апгрейднуть приложение на новую версию Ruby on Rails. Эти шаги также доступны в отдельных руководствах по релизам.
---
...
Подробности смотрите в руководстве [по кэшированию с Rails](/docs/caching-with-rails#connection-pool-options).
...
Альтернативно можно настроить `I18n.exception_handler`. Подробности смотрите в [руководстве i18n](/docs/i18n#using-different-exception-handlers).
...
Подробности смотрите в руководстве [Тестирование приложений на Rails](/docs/testing#running-tests-in-continuous-integration-ci).
...
Смотрите руководство [Configuring Rails Applications](/docs/configuring#config-active-record-encryption-hash-digest-class) о подробностях по `config.active_record.encryption.hash_digest_class`.

В дополнение была представлена новая конфигурация [`config.active_record.encryption.support_sha1_for_non_deterministic_encryption`](/docs/configuring#config-active-record-encryption-support-sha1-for-non-deterministic-encryption) чтобы починить [баг](https://github.com/rails/rails/issues/42922), вызывающий то, что некоторые атрибуты были зашифрованы с помощью SHA-1, даже если был настроен SHA-256 с помощью вышеупомянутой конфигурации `hash_digest_class`.
...
* `:rescuable` - рендерить HTML страницы ошибки для исключений, объявленных в [`config.action_dispatch.rescue_responses`](/docs/configuring#config-action-dispatch-rescue-responses)
...
## Апгрейд с Rails 6.1 на Rails 7.0
...
[`config.cache_classes`]: /docs/configuring#config-cache-classes
...
Если вы все еще получаете это предупреждение в логах, обратитесь к разделу об автоматической загрузки при запуске приложения в [руководстве по автозагрузке](/docs/autoloading-and-reloading-constants#autoloading-when-the-application-boots). В противном случае вы получите `NameError` в Rails 7.
...
[`config.autoload_once_paths`]: /docs/configuring#config-autoload-once-paths
...
<Callout type="info">
При первом дампе схемы с Rails 7.0, вы увидите множество изменений в этот файл, включая некоторую информацию о столбцах. Убедитесь, что вы просмотрели содержимое файла новой схемы, и отправили его в свой репозиторий.
</Callout>

## Апгрейд с Rails 6.0 на Rails 6.1
...
## Апгрейд с Rails 5.2 на Rails 6.0
...
[`config.ssl_options`]: /docs/configuring#config-ssl-options
...
Если в вашем приложении есть Single Table Inheritance, посмотрите на [раздел о наследовании с единой таблицей](/docs/autoloading-and-reloading-constants#single-table-inheritance) руководства Автозагрузка и перезагрузка констант (режим Zeitwerk).
...
[`config.add_autoload_paths_to_load_path`]: /docs/configuring#config-add-autoload-paths-to-load-path
#### Тредобезопасность
...
[`config.active_storage.replace_on_assign_to_many`]: /docs/configuring#config-active-storage-replace-on-assign-to-many
...
[`config.exceptions_app`]: /docs/configuring#config-exceptions-app
## Апгрейд с Rails 5.1 на Rails 5.2
...
## Апгрейд с Rails 5.0 на Rails 5.1
...
## Апгрейд с Rails 4.2 на Rails 5.0
...
[`config.action_mailer.perform_caching`]: /docs/configuring#config-action-mailer-perform-caching
...
## Апгрейд с Rails 4.1 на Rails 4.2
...
## Апгрейд с Rails 4.0 на Rails 4.1
...
<Callout type="info">
Ваши собственные теги `<script>` трактуются как межсайтовые и по умолчанию также блокируются. Если вы действительно хотите загружать JavaScript в тегах `<script>`, теперь нужно явно отключить защиту CSRF для этого экшна.
</Callout>
...
<Callout type="info">
Пользовательские задачи rake по умолчанию будут запущены в окружении `development`. Если хотите запускать их в других средах, проконсультируйтесь со [Spring README](https://github.com/rails/spring#rake).
</Callout>
...
<Callout type="warn">
Нельзя просто заменить `MultiJson.dump` и `MultiJson.load` на `JSON.dump` и `JSON.load`. Эти API гема JSON означают сериализацию и десериализацию произвольных объектов Ruby, и, в основном, [небезопасны](https://www.ruby-doc.org/stdlib-2.2.2/libdoc/json/rdoc/JSON.html#method-i-load).
</Callout>
...
### Изменения в скоупах по умолчанию
...
### Рендеринг содержимого из строки
...
## Апгрейд с Rails 3.2 на Rails 4.0
...
[`config.assets.precompile`]: /docs/configuring#config-assets-precompile
### Active Support
...
Метод кэширования изменился между Rails 3.x и 4.0. Вы должны [изменить пространство имен](/docs/caching-with-rails#activesupport-cache-store) и развертывать с холодным кэшем.
...
[`config.assets.js_compressor`]: /docs/configuring#config-assets-js-compressor
### sass-rails
...
Снова, большая часть изменений относится к конвейеру ресурсов. Подробнее о них можно прочитать в руководстве [Asset Pipeline](/docs/asset-pipeline).
