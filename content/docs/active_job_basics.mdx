---
title: Основы Active Job
description: Это руководство даст вам все, что нужно, чтобы начать создавать, ставить в очередь и выполнять фоновые задания.
---
...
[`config.active_job.queue_adapter`]: /docs/configuring#config-active-job-queue-adapter
### Запуск бэкенда
...
[`config.active_job.queue_name_delimiter`]: /docs/configuring#config-active-job-queue-name-delimiter
[`config.active_job.queue_name_prefix`]: /docs/configuring#config-active-job-queue-name-prefix
...
### Колбэки массового добавления в очередь
...
Однако метод `perform_all_later` вызывает событие [`enqueue_all.active_job`](/docs/active_support_instrumentation#enqueue-all-active-job), на которое вы можете подписаться с помощью `ActiveSupport::Notifications`.
...
Если вам нужна помощь в том, чтобы понять, откуда берутся задания, вы можете включить [подробное логирование](/docs/debugging_rails_applications#verbose-enqueue-logs).
