...
Однако метод `perform_all_later` вызывает событие [`enqueue_all.active_job`](/docs/active-support-instrumentation#enqueue-all-active-job), на которое вы можете подписаться с помощью `ActiveSupport::Notifications`.
...
Если вам нужна помощь в том, чтобы понять, откуда берутся задания, вы можете включить [подробное логирование](/docs/debugging-rails-applications#verbose-enqueue-logs).
