---
title: Active Record для PostgreSQL
description: Данное руководство рассказывает о специфике использования PostgreSQL с Active Record.
---
...
Для начала работы с PostgreSQL почитайте руководство [Конфигурирование приложений на Rails](/docs/configuring#konfigurirovanie-bazy-dannyh-postgresql).
...
<Callout type="info">
Значения enum нельзя удалять. Можно прочесть почему [здесь](http://www.postgresql.org/message-id/29F36C7C98AB09499B1A209D48EAA615B7653DBC8A@mail2a.alliedtesting.com).
</Callout>
...
<Callout type="info">
Если используете PostgreSQL более ранний, чем версия 13.0, необходимо включить специальные расширения для использования UUID. Включите расширение `pgcrypto` (PostgreSQL >= 9.4) или расширения `uuid-ossp` (для более ранних релизов).
</Callout>
...
<Callout type="info">
Генерируемые столбцы поддерживаются, начиная с 12.0 версии PostgreSQL.
</Callout>
