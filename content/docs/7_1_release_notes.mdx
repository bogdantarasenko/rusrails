---
title: Заметки о релизе Ruby on Rails 7.1
description: "Ключевые новинки в Rails 7.1:"
---
...
`ActiveSupport::MessagePack` можно использовать в качестве [сериализатора сообщения](/docs/configuring#config-active-support-message-serializer):
...
В качестве [сериализатора куки](/docs/configuring#config-action-dispatch-cookies-serializer):

```ruby
config.action_dispatch.cookies_serializer = :message_pack
```

И в качестве [сериализатора кэша](/docs/caching_with_rails#configuration):
