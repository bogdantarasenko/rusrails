---
title: Rails on Rack
description: Это руководство раскрывает интеграцию Rails и Rack и взаимодействие с другими компонентами Rack.
---
...
[`config.middleware`]: /docs/configuring#config-middleware
...
* Защищает от атак переназначения DNS, явно разрешая хосты, с которых может быть послан запрос. Смотрите инструкции в [руководстве по конфигурированию](/docs/configuring#actiondispatch-hostauthorization).
...
[`config.public_file_server.enabled`]: /docs/configuring#config-public-file-server-enabled
**`Rack::Lock`**
...
[`config.session_store`]: /docs/configuring#config-session-store
