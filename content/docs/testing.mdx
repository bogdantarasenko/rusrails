...
Если помните, мы использовали команду `bin/rails generate model` в руководстве [Rails для начинающих](/docs/getting_started). Мы создали нашу первую модель, где, среди прочего, создались незаконченные тесты в папке `test`:
...
Если работаете со [связями](/docs/association_basics), можно определить ссылку между двумя различными фикстурами. Вот пример для связи `belongs_to`/`has_many`:
...
<Callout type="info">
Если выполнены шаги в разделе про [базовую аутентификацию](/docs/getting_started#basic-authentication), то необходимо добавить авторизацию в заголовок каждого запроса, чтобы все тесты проходили:
</Callout>
