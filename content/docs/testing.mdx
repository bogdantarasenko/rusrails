---
title: Тестирование приложений на Rails
description: Это руководство раскрывает встроенные в Rails механизмы для тестирования вашего приложения.
---
...
Если помните, мы использовали команду `bin/rails generate model` в руководстве [Rails для начинающих](/docs/getting-started). Мы создали нашу первую модель, где, среди прочего, создались незаконченные тесты в папке `test`:
...
[`config.active_support.test_order`]: /docs/configuring#configuring-active-support
...
<Callout type="info">
За подробностями о `Minitest` обратитесь к [его документации](http://docs.seattlerb.org/minitest)
</Callout>
...
### Запуск тестов в Непрерывной Интеграции (CI)
...
Если работаете со [связями](/docs/active-record-associations), можно определить ссылку между двумя различными фикстурами. Вот пример для связи `belongs_to`/`has_many`:
...
По умолчанию Rails пытается обработать исключения, возникшие во время тестов, и отобразить страницы ошибок в формате HTML. Это поведение можно контролировать с помощью настройки [`config.action_dispatch.show_exceptions`](/docs/configuring#config-action-dispatch-show-exceptions).
...
<Callout type="info">
Если выполнены шаги в разделе про [базовую аутентификацию](/docs/getting-started#basic-authentication), то необходимо добавить авторизацию в заголовок каждого запроса, чтобы все тесты проходили:
</Callout>
...
#### Тестирование электронных писем в очереди
