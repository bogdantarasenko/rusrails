---
title: Валидации Active Record
description: Это руководство научит, как осуществлять валидацию состояния объектов до того, как они будут направлены в базу данных, используя особенность валидаций Active Record.
---
...
<Callout type="info">
Чтобы просмотреть список доступных хелперов по умолчанию, обратитесь к [`ActiveModel::Validations::HelperMethods`][].
</Callout>
...
<Callout type="info">
Эта проверка выполняется, только если `email_confirmation` не равно `nil`. Чтобы требовать подтверждение, нужно добавить еще проверку на существование проверяемого атрибута (мы рассмотрим `presence` [чуть позже](#presence)):
</Callout>
...
<Callout type="info">
Этот валидатор требует предоставления опции сравнения. Каждая опция принимает значение, proc или символ. Любой класс, включающий Comparable, может быть сравнен.
</Callout>
...
<Callout type="info">
Одновременно может быть использована одна опция, кроме опций `:minimum` и `:maximum`, которые комбинируются вместе.
</Callout>
...
<Callout type="info">
По умолчанию `numericality` не допускает значения `nil`. Чтобы их разрешить, можно использовать опцию `allow_nil: true`. Отметьте, что для столбцов `Integer` и `Float` пустые строки конвертируются в `nil`.
</Callout>
...
<Callout type="info">
Если хотите убедиться, что связь и существует, и валидна, нужно также использовать `validates_associated`. Подробнее [ниже](#validates-associated)
</Callout>
...
<Callout type="info">
Если желаете убедиться, что связь и существует, и валидна, также необходимо использовать `validates_associated`. Подробнее [ниже](#validates-associated)
</Callout>
...
<Callout type="info">
[`validates_associated`][] можно использовать только с объектами ActiveRecord, а все до этого можно было также использовать с любым объектом, включающим [`ActiveModel::Validations`][].
</Callout>
...
<Callout type="info">
Ошибки, добавляемые в `record.errors[:base]` относятся к состоянию записи в целом.
</Callout>
...
<Callout type="info">
Не все из этих опций поддерживаются каждым валидатором, обратитесь к документации API для [`ActiveModel::Validations`][].
</Callout>
...
Мы раскроем больше способов использования для `on:` в [руководстве по колбэкам](/docs/active_record_callbacks).
...
Метод [`full_message`][] генерирует более дружелюбное сообщение с добавлением имени атрибута с большой буквы вначале. (Чтобы настроить формат, используемый `full_message`, посмотрите [руководство I18n](/docs/i18n#active-model-methods).)
...
## Отображение ошибок валидации во вью
