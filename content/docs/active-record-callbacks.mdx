---
title: Колбэки Active Record
description: Это руководство научит вас вмешиваться в жизненный цикл ваших объектов Active Record.
---
...
В результате обычных операций приложения на Rails, объекты могут быть [созданы, обновлены и уничтожены](/docs/active-record-basics#crud-reading-and-writing-data). Active Record дает возможность вмешаться в этот жизненный цикл объекта, таким образом, вы можете контролировать свое приложение и его данные.
...
### Регистрация колбэков, срабатывающих на событиях жизненного цикла
...
<Callout type="info">
У колбэков `after_initialize` и `after_find` нет пары `before_*`.
</Callout>
...
<Callout type="info">
Методы `find_by_*` и `find_by_*!` это динамические методы поиска, генерируемые автоматически для каждого атрибута. Изучите подробнее их в [разделе Динамический поиск](/docs/active-record-querying#dynamic-finders)
</Callout>

## Условные колбэки

Как и в [валидациях](/docs/active-record-validations), возможно сделать вызов метода колбэка условным в зависимости от заданного предиката. Это осуществляется при использовании опций `:if` и `:unless`, которые могут принимать символ, `Proc` или массив.
...
Как и в [валидациях](/docs/active-record-validations), возможно пропустить колбэки с помощью следующих методов:
...
<Callout type="warn">
При использовании колбэка `before_destroy` его следует размещать перед связями с `dependent: :destroy` (или использовать опцию `prepend: true`), чтобы гарантировать их выполнение до того, как записи будут удалены с помощью `dependent: :destroy`.
</Callout>
...
<Callout type="info">
Опция `:on` определяет, когда будет запущен колбэк. Если не предоставить опцию `:on`, колбэк будет запущен для каждого события жизненного цикла.  Подробнее об `:on` читайте [тут](#registering-callbacks-to-fire-on-life-cycle-events).
</Callout>
...
Если по какой-то причине вы по-прежнему хотите, чтобы они выполнялись в обратном порядке, вы можете установить [следующую конфигурацию](/docs/configuring#config-active-record-run-after-transaction-callbacks-in-order-defined) в значение `false`. Тогда колбэки будут выполняться в обратном порядке.
...
<Callout type="info">
Это также применяется ко всем вариациям `after_*_commit`, таким как `after_destroy_commit`.
</Callout>
