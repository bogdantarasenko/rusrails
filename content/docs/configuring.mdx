---
title: Конфигурирование приложений на Rails
description: Это руководство раскрывает особенности конфигурирования и инициализации, доступные приложениям на Rails.
---
...
Пожалуйста, за подробностями обратитесь к [руководству по автозагрузке](/docs/autoloading_and_reloading_constants).
...
Принимает массив путей, по которым Rails будет автоматически загружать константы. По умолчанию пустой массив. Начиная с [Rails 6](/docs/upgrading_ruby_on_rails#autoloading) не рекомендуется настраивать это. Подробнее смотрите в руководстве [Автозагрузка и перезагрузка констант](/docs/autoloading_and_reloading_constants#autoload-paths)
...
Смотрите [Добавление Nonce](/docs/security#adding-a-nonce) в руководстве по безопасности
...
Смотрите [Отчет о нарушениях](/docs/security#reporting-violations) в руководстве по безопасности
...
*DEPRECATED*: Следует использовать [учетные данные](/docs/security#custom-credentials) вместо зашифрованных секретов.

Когда `true`, попытается прочитать зашифрованные секреты из `config/secrets.yml.enc`

#### `config.relative_url_root`

Можно использовать, чтобы сообщить Rails, что вы [развертываете в поддиректорию](/docs/configuring#deploy-to-a-subdirectory-relative-url-root). По умолчанию `ENV['RAILS_RELATIVE_URL_ROOT']`.
...
Конфигурирует, должен ли применяться метод `deep_munge` на параметрах. Подробнее смотрите в руководстве [Безопасность приложений на Rails](/docs/security#unsafe-query-generation). По умолчанию `true`.
...
Конфигурация `config.action_dispatch.show_exceptions` управляет тем, как Action Pack (а точнее, промежуточная программа [`ActionDispatch::ShowExceptions`](/docs/configuring#actiondispatch-showexceptions)) обрабатывает исключения, возникающие при ответе на запросы.
...
Значение `:rescueable` настраивает Action Pack на перехват исключений, определенных в [`config.action_dispatch.rescue_responses`](/docs/configuring#config-action-dispatch-rescue-responses), а все остальные - вызывать. Например, Action Pack перехватит `ActiveRecord::RecordNotFound`, но вызовет `NoMethodError`.

Значение `:none` настраивает Action Pack на вызов всех исключений.

* `:all` - отображать страницы ошибок для всех исключений
* `:rescuable` - отображать страницы ошибок для исключений, объявленных в [`config.action_dispatch.rescue_responses`](/docs/configuring#config-action-dispatch-rescue-responses)
...
Определяет метод доставки, по умолчанию `:smtp`. За подробностями обращайтесь к разделу по настройке в руководстве [Основы Action Mailer](/docs/action_mailer_basics#action-mailer-configuration)
...
<Callout type="warn">
`Marshal` это потенциальное направление атак десериализации в случаях, когда утек секрет подписания сообщения. _Если возможно, выбирайте сериализатор, не поддерживающий `Marshal`._
</Callout>

<Callout type="info">
Сериализаторы `:message_pack` и `:message_pack_allow_marshal` поддерживают некоторые типы Ruby, не поддерживаемые JSON, такие как `Symbol`. Они также предоставляют улучшенное быстродействие и меньший размер полезной нагрузки. Однако они требуют [гема `msgpack`](https://rubygems.org/gems/msgpack).
</Callout>
...
Конфигурационные опции описаны подробнее в [Обзор Action Cable](/docs/action_cable_overview#configuration).
