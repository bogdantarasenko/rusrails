...
Чтобы понять Active Model, вам нужно немного знать об [Active Record](/docs/active-record-basics). Active Record - это ORM (Object Relational Mapper), который соединяет объекты, данные которых требуют постоянного хранения, с реляционной базой данных. Однако он обладает функциональностью, которая полезна вне ORM, например, валидация, колбэки, переводы, возможность создавать пользовательские атрибуты и т.д.
...
[`ActiveModel::API`](https://api.rubyonrails.org/classes/ActiveModel/API.html) добавляет возможность классу работать с [Action Pack](https://api.rubyonrails.org/files/actionpack/README_rdoc.html) и [Action View](/docs/action-view-overview) прямо из коробки.
...
<Callout type="info">
Дополнительную информацию об использовании `form_with` и `render` с объектами, совместимыми с `ActiveModel::API`, можно найти в руководствах [по хелперам форм](/docs/form-helpers) и [по макетам и рендерингу](layouts-and-rendering) соответственно.
</Callout>
